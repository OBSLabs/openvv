...
function baseGetBrowserDetailsByUserAgentTest(userAgent, broswerObj) {
    ...
    //Act
    var $ovvvpaid = new window.OVV();

    //Assert
    deepEqual($ovvvpaid.browser, broswerObj, 'wrong broswer object.');
}
...
test("Test one subscriber", function () {
    ...
    var uid = 10;
    $ovvvpaid.subscribe(events, uid, funcTrue);

    //Act
    $ovvvpaid.publish(eventName, uid);
...
test("Test subscribe to two different events", function () {
    ...
    var uid = 10;
    $ovvvpaid.subscribe(events, uid, funcTrue1);

    //Act
    $ovvvpaid.publish(eventName1, uid, '1');
    $ovvvpaid.publish(eventName2, uid, '2');
...
test("Test subscribe twice to the same event", function () {
    ...
    var uid1 = 10;
    var uid2 = 11;
    $ovvvpaid.subscribe(events, uid1, funcTrue1);
    $ovvvpaid.subscribe(events, uid2, funcTrue1);

    //Act
    var publishResult1 = $ovvvpaid.publish(eventName1, uid1, '1');
    var publishResult1 = $ovvvpaid.publish(eventName1, uid2, '2');
...
test("Test publish not exist event name", function () {
    ...
    //Act
    $ovvvpaid.publish('notExistEventName', uid);
...
test("Test publish not exist uid", function () {
    ...
    //Act
    var publishResult1 = $ovvvpaid.publish(eventName1, 'notExistUID');
...
test("Test publish null as the uid", function () {
    ...
    //Act
    $ovvvpaid.publish(eventName1, null);
...
test("Test publish undefined as the uid", function () {
    ...
    //Act
    $ovvvpaid.publish(eventName1, undefined);
...
test("Test publish undefined as the event name", function () {
    ...
    //Act
    $ovvvpaid.publish(undefined, uid);
...
test("Test previous events", function () {
    ...
    //Act
    $ovvvpaid.subscribe(events, uid, funcTrue1, true);
...
test("Test previous events when the subscribe event doesn't have and pervious events", function () {
    ...
    var events = [eventName];
    var uid = 10;
    $ovvvpaid.publish(otherEventName, uid);
...
test("Test previous events with two buffered event verify the order of the events", function () {
    ...
    var events = [eventName];
    var uid = 10;
    $ovvvpaid.publish(eventName, uid, '1');
    $ovvvpaid.publish(eventName, uid, '2');
...
test("Test previous events with several different buffered event verify the order of the events", function () {
    ...
    $ovvvpaid.publish(eventName1, uid, '1');
    $ovvvpaid.publish(eventName2, uid, '2');
    $ovvvpaid.publish(eventName3, uid, '3');
    $ovvvpaid.publish(eventName4, uid, '4');
